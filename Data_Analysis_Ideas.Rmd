---
title: "Data_Results_Ideas"
date: "10/18/2017"
output: html_document
---

## Data and Results Presentation Ideas:

#### Goal: Present in a meaningful and concise way the index hopping landscape of our data...

The outcome of the experiment indicated that a vast majority of our reads were *not* affected by index hopping, especially those of high quality (sic). To prove this, we could sort the reads that had index swapping from our reads that did not (basically doing what Pete ultimately wanted us to do in the original index hopping assignment) by modifying the python code of the last summer assignment a little. See below (and in the github repo):
...
The output files here could be run through FastQC to get quality profiles on basically two groups: Reads with Index Hopping and Reads without. These graphs could be included in the paper/presentation:

```{BASH, eval=FALSE}
ml fastqc/0.11.5
>$ fastqc --noextract -o Known_QC_analysis /{path}/{to}/{files}/Known_index_pair_R1.fastq /{path}/{to}/{files}/Known_index_pair_R4.fastq
>$fastqc --noextract -o Known_QC_analysis /{path}/{to}/{files}/Unknown_index_pair_R1.fastq /{path}/{to}/{files}/Unknown_index_pair_R4.fastq
```


Additionally, we could look at what index-hopping pairs most often occured; here is a dot-plot representation of that using the counts per index pair file produced out of the Index Hopping python script:
...
```{r, echo=FALSE}
# Scratch work to determine statistics about read qualities:
Known_R1_quals <- read.table('Use_this.txt', header = F, sep = '\t')
Known_R4_quals <- read.table('Use_this_3.txt', header = F, sep = '\t')
Unknown_R1_quals <- read.table('Use_this_2.txt', header = F, sep = '\t')
Unknown_R4_quals <- read.table('Use_this_4.txt', header = F, sep = '\t')

ave_R1 <- mean(Known_R1_quals$V1)
sd_R1 <- sd(Known_R1_quals$V1)
ave_u_R1 <- mean(Unknown_R1_quals$V1)
sd_u_R1 <- sd(Unknown_R1_quals$V1)
ave_R4 <- mean(Known_R4_quals$V1)
sd_R4 <- sd(Known_R4_quals$V1)
ave_u_R4 <- mean(Unknown_R4_quals$V1)

for_test <- read.table('T-test.txt', header = F, sep = '\t')
colnames(for_test) <- c("status", "mean")
t.test(mean ~ status, data = for_test)
```